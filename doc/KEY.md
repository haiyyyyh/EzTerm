...
这是保证EzTerm轻量、简洁做出的决策，也能防止使用EzTerm的开发者所编写的程序，出现可移植性极差和快捷键体系混乱的问题  
总的来说，我希望通过硬性的支持约束来收紧开发者的行为，让使用者们能够意识到某些快捷键不应被使用，如果需要请自行定义  
...
### 功能键
> 指`F1`～`F12`，以及`pgup`、`pgdn`、`ins`、`del`、`home`、`end`，还有方向键
- 都有对应的转义序列
...
这些键在最开始都有着明确的定义，将他们组合在一起基本会得到你能预测的效果  
但是，后来GUI软件模糊了这一点，人们只知道这几个是修饰键，以及由它们构成的组合键，但不知道本质  
...
`Ctrl` ：一说为control，他本质上是会将按下的26字母键 映射到ascii表中的控制字符区域，通过组合键实现控制字符输入，我想这有必要好好讲讲  
